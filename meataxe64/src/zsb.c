...
static void cat_matrices(const char *from1, const char *from2, const char *to, const char *tmp_bs)
{
  ...
  fRowRiffle(tmp_bs, 1, from1, 1, from2, 1, to, 1);
}
...
int main(int argc, const char *argv[])
{
  ...
  res = fech(in_vecs, 0, row_sel, 1,
             zero_bs, 1, "NULL", 1,
             "NULL", 1, in_vecs_rem, 1);
  /* fail if rank 0 */
  if (0 == res) {
  ...
  while (mrank < rank && rank < nor) {
    ...
    for (i = 0; i < ngens; i++) {
      ...
      if (0 != mul_gen->next_tbd.size) {
        ...
        if (first) {
          /* Just echelise this */
          res = fech(clean_tmp1, 1, row_sel, 1,
                     ech_tmp_bs, 1, "NULL", 1,
                     "NULL", 1, ech_tmp_rem, 1);
          if (0 == res) {
            remove(ech_tmp_bs);
            remove(ech_tmp_rem);
          }
          ...
          /* Then echelise */
          res = fech(clean_tmp2, 1, row_sel, 1,
                     ech_tmp_bs1, 1, "NULL", 1,
                     "NULL", 1, ech_tmp_rem1, 1);
          if (0 != res) {
