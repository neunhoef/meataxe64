/*
 * Vector to padded vector. Allow a vector to be put in a form
...
int main(int argc, const char * const argv[])
{
  ...
  FILE *inp = NULL;
  FILE *outp = NULL;
  u32 prime, nob, noc, nor, len, nod, cols_out, pcols_out, noc_out, rows_out, len_out, i, j;
  ...
  pcols_out = padded_cols(cols_out, nob);
  noc_out = (noc / cols_out) * pcols_out;
  rows_out = noc / cols_out;
  /*
   * Allocate half the memory to the input row,
   * and the other half to the output row.
   */
  ...
  h_out = header_create(prime, nob, nod, noc_out, nor);
  /* Although we're outputing a long row, it will be a concatenation of short rows */
  len_out = compute_len(nob, cols_out);
