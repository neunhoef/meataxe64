/*
 * $Id: zcv32to64.c,v 1.4 2006/12/04 22:33:24 jon Exp $
 *
...
static const char *name = "zcv32to64";
...
int main(int argc, const char * const argv[])
{
  ...
  if (U32PRIME_BIT != (header_get_raw_prime(h) & PRIME_BITS)) {
    fclose(inp);
    header_free(h);
    fprintf(stderr, "%s: %s is not a 32 bit meataxe file\n", name, in);
    exit(1);
  }
  ...
  header_set_raw_prime((header *)h, prime | U64PRIME_BIT);
  ...
  if (1 == prime) {
    ...
    u32 *row_in = my_malloc(u32len * sizeof(u32));
    u64 *row_out = my_malloc(u64len * sizeof(u64));
    for (i = 0; i < nor; i++) {
      ...
      for (j = 0; j < noc; j++) {
        u32 elt = get_element_from_u32_row(nob, j, row_in);
        put_element_to_u64_row(nob, j, row_out, elt);
      }
