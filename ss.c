/*
 * $Id: ss.c,v 1.1 2001/11/25 00:17:19 jon Exp $
 *
              ...
              const char *out, const char *name)
{
  ...
  inp1 = fopen(range, "rb");
  inp2 = fopen(image, "rb");
  if (NULL == inp1 || NULL == inp2) {
    fprintf(stderr, "%s: cannot open one of %s, %s, terminating\n", name, range, image);
    cleanup(inp1, inp2);
    exit(1);
  }
  if (0 == read_binary_header(inp1, &h_in1, name) ||
      0 == read_binary_header(inp2, &h_in2, name)) {
