/*
 * $Id: tmul.c,v 1.11 2021/08/02 18:19:40 jon Exp $
 *
...
int tmulp(const char *m1, const char *m2, const char *m3,
          const char *m4, const char *name)
{
  FILE *in1 = NULL, *in2 = NULL, *in3 = NULL, *out = NULL;
  const header *h_in, *h1, *h2;
  ...
  nor2 = header_get_nor(h2);
  len2 = header_get_len(h2);
  /* Check the tensor factors are square
  ...
  if (noc1 != nor1 ||
      ...
      (nob != header_get_nob(h1) && 0 == is_map1) ||
      (nob != header_get_nob(h2) && 0 == is_map2)) {
    fprintf(stderr, "%s: incompatible parameters (1) for %s, %s, %s, terminating\n",
            name, m1, m2, m3);
    cleanup(in1, in2, in3);
    return 0;
  }
  header_free(h1);
  header_free(h2);
  /* Now check noc1 * padded(noc2) == noc */
  if (noc1 * padded_cols(noc2, nob) != noc) {
    fprintf(stderr, "%s: incompatible parameters (2) for %s, %s, %s, terminating\n",
            name, m1, m2, m3);
    cleanup(in1, in2, in3);
    return 0;
  }
  ...
  while (i < nor) {
    ...
    errno = 0;
    /*printf("Reading %u rows from input\n", to_do);*/
    if (0 == endian_read_matrix(in1, m_ptrs1, len2, nor1 * to_do)) {
      if ( 0 != errno) {
        perror(name);
      }
      fprintf(stderr, "%s: failed to read row %u from %s, terminating\n",
    ...
    NOT_USED(len);
    errno = 0;
    if (0 == endian_write_matrix(out, m_ptrs1, len2, nor1 * to_do)) {
      if ( 0 != errno) {
        perror(name);
      }
      fprintf(stderr, "%s: failed to write output row %u to %s, terminating\n",
