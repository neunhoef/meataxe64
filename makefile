...
#
# $Id: makefile,v 1.91 2004/08/22 11:09:47 jon Exp $
#
...
LIB_MODULES=	add base clean count clean_file clean_vectors command conj dets diff diffd elements endian exrows extend extend_matrix files grease header ident iv ivf join lv map map_or_row maps matrix memory msb msbf msp mspf mul mv ns nsf orbit parse pco pcv powers primes project ps qs rand read restrict rn rnf scale script sign signf singular span spanmsp ss ss_map sums sumsf sums_utils symb system tco tcv te tmul tra tsp tspf ttr utils vp write
...
ZIV_MODULES=	rows ziv
ZIVF_MODULES=	rows zivf
ZJOIN_MODULES=	rows zjoin
ZLP_MODULES=	rows zlp
ZLV_MODULES=	rows zlv
...
ZSIGN_MODULES=	rows zsign
ZSIGNF_MODULES=	rows zsignf
